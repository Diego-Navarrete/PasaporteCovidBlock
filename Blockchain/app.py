...
def nueva_transaccion():
    ...
    required = ['origen', 'TipoD', 'numero', 'nombreapellido', 'ciudad', 'regimen', 'eps', 'centroa', 'fechap', 'resultado', 'tipov', 'lugar', 'FechaP', 'FechaS']

    if not all(k in values for k in required ):
        return "datos de transaccion invalidos ", 404
    index = Blockchain.add_new_transaction(values['origen'],values['TipoD'],values['numero'], values['nombreapellido'], values['ciudad'], values['regimen'], values['eps'], values['centroa'], values['fechap'], values['resultado'], values['tipov'], values['lugar'], values['FechaP'], values['FechaS']) 
...
def crear_chain_desde_papelera(chain_papelera):
    ...
    for idx, block_data in enumerate(chain_papelera):
        ...
        block = Block(block_data["index"],
                      ...
                      block_data["nonce"])
        proceso = block_data['hash']
        adicional = generar_blockchain.add_block(block, proceso)
        if not adicional:
