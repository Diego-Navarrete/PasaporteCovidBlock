...
class Block:
    def __init__(self, index, transacciones, timestamp, previous_hash, nonce=0):
        ...
        self.nonce = nonce # Proceso que se ha realizado un cambio para obtener el hashs
...
class Blockchain:
    ...
    difficulty = 3 # Dificultad del algoritmo hash
    ...
    def add_block(self, block, Proceso): # La funcion que agrega el bloque a la cadena despues de la verificacion
        ...
        if not Blockchain.is_valid_Proceso(block, Proceso):  
            return False
    ...
    def add_new_transaction(self,origen, TipoD, numero, nombreapellido, ciudad, regimen, eps, centroa, fechap, resultado, tipov, lugar, FechaP, FechaS,operacion, datos):

        self.unconfirmed_transactions.append({
            
            'TIPO_DOCUMENTO': TipoD,
            'NUMERO_DOCUMENTO': numero, 
            "NOMBE_APELLIDO": nombreapellido,
            "CIUDAD" : ciudad, 
            "REGIMEN": regimen, 
            "EPS": eps, 
            "CENTRO_ATENCION": centroa,
            "FECHA_PRUEBA": fechap, 
            "RESULTADO": resultado,
            "TIPO_VACUNA": tipov, 
            "LUGAR_VACUNACION": lugar, 
            "FECHA_PRIMERA_DOSIS": FechaP,
            "FECHA_SEGUNDA_DOSIS": FechaS,
            }
            )
    ...
    def mine(self): # Esta funcion sirve como interfaz para agregar la transaccion a la cadena de bloques y minarlos
        ...
        Proceso = self.Proceso_de_trabajo(new_block)
        self.add_block(new_block, Proceso)
